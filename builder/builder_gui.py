...
class BuilderGUI:
    ...
    def _setup_ui(self):
        ...
        self.theme_btn = ttk.Button(top_frame, text="切换主题 (Dark/Light)", command=self._toggle_theme)
        ...
        self.preview_btn = ttk.Button(btn_frame, text="👁 生成前预览", command=self._preview_ui)
        self.preview_btn.pack(side='left', padx=5, expand=True, fill='x', ipady=5)
        
        self.build_btn = ttk.Button(btn_frame, text="🚀 开始生成", command=self._start_build_thread)
